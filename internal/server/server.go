package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-ironworks/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-ironworks"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/artifacts",s.handleList)
    s.mux.HandleFunc("POST /api/artifacts",s.handleCreate)
    s.mux.HandleFunc("POST /api/artifacts/{id}/download",s.handleDownload)
    s.mux.HandleFunc("DELETE /api/artifacts/{id}",s.handleDelete)
