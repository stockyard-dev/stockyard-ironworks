...
async function load(){var r=await fetch(A+'/artifacts').then(function(r){return r.json()});items=r.artifacts||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.build_id||'').toLowerCase().includes(q)||(i.version||'').toLowerCase().includes(q)||(i.platform||'').toLowerCase().includes(q)});
...
if(i.build_id)h+='<div class="item-sub">'+esc(i.build_id)+'</div>';
...
if(i.version)h+='<span>'+esc(i.version)+'</span>';
if(i.platform)h+='<span>'+esc(i.platform)+'</span>';
if(i.size_bytes)h+='<span>'+i.size_bytes+'</span>';
if(i.checksum)h+='<span>'+esc(i.checksum)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.download_url)h+='<span>'+esc(i.download_url)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/artifacts/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",build_id:"",version:"",platform:"",size_bytes:0,checksum:"",status:"",download_url:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' IRONWORKS</h2>';
h+='<div class="fr"><label>Name *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
h+='<div class="fr"><label>Build I D</label><input id="f-build-id" value="'+esc(i.build_id)+'"></div>';
h+='<div class="fr"><label>Version</label><input id="f-version" value="'+esc(i.version)+'"></div>';
h+='<div class="fr"><label>Platform</label><input id="f-platform" value="'+esc(i.platform)+'"></div>';
h+='<div class="fr"><label>Size Bytes</label><input id="f-size-bytes" type="number" value="'+(i.size_bytes||0)+'"></div>';
h+='<div class="fr"><label>Checksum</label><input id="f-checksum" value="'+esc(i.checksum)+'"></div>';
h+='<div class="fr"><label>Status</label><input id="f-status" value="'+esc(i.status)+'"></div>';
h+='<div class="fr"><label>Download U R L</label><input id="f-download-url" value="'+esc(i.download_url)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),build_id:document.getElementById('f-build-id').value.trim(),version:document.getElementById('f-version').value.trim(),platform:document.getElementById('f-platform').value.trim(),size_bytes:parseInt(document.getElementById('f-size-bytes').value)||0,checksum:document.getElementById('f-checksum').value.trim(),status:document.getElementById('f-status').value.trim(),download_url:document.getElementById('f-download-url').value.trim()};
if(editId){await fetch(A+'/artifacts/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/artifacts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
